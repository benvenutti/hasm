...
struct FixtureLabel {
  const std::regex label{Hasm::Hack::Grammar::LABEL};
};

BOOST_FIXTURE_TEST_SUITE(label, FixtureLabel)

BOOST_AUTO_TEST_CASE(test_validLabel) {
  BOOST_CHECK(std::regex_match("(A)", label));
  BOOST_CHECK(std::regex_match("(AAAAAAA)", label));
}

BOOST_AUTO_TEST_CASE(test_invalidLabel_missingId) {
  BOOST_CHECK_EQUAL(std::regex_match("()", label), false);
}

BOOST_AUTO_TEST_CASE(test_invalidLabel_missingLeftParen) {
  BOOST_CHECK_EQUAL(std::regex_match("A)", label), false);
}
